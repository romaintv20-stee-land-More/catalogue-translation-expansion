...
targets=[
 ...
 dict(loader='fabric',mc='26.3',fabric='>=0.19.5',cat='>=1.12.3 <1.13',pack=[97,1],keys=G6,locales=MASTER,protect={c:set(G1) for c in ['de_de','pl_pl','zh_cn']} | {'ru_ru':set(G6)-{'catalogue.gui.missing_branding','catalogue.gui.missing_branding.desc'}}),
 dict(loader='neoforge',mc='26.3',neo='[26.3.0.3-beta,26.4)',cat='[1.12.3,1.13)',pack=[97,1],keys=G6,locales=MASTER,protect={c:set(G1) for c in ['de_de','pl_pl','zh_cn']} | {'ru_ru':set(G6)-{'catalogue.gui.missing_branding','catalogue.gui.missing_branding.desc'}}),
]

def pack_mcmeta(t):
    if t['mc']=='26.3':
        return json.dumps({'pack':{'description':f'{NAME} {VERSION}',
...
def neo_toml(t):
    mc_range='[26.3,26.4)' if t['mc']=='26.3' else '[1.20.4,27)'
    ...
    for r in report:
        with zipfile.ZipFile(DIST/r['file']) as z:
            ...
            if r['minecraft']=='26.3':
                ...
                assert 'assets/minecraft/lang/mr_in.json' not in names
                assert 'assets/catalogue/lang/mr_in.json' not in names
                pack=json.loads(z.read('pack.mcmeta'))
                assert pack['pack']['min_format']==[97,1] and pack['pack']['max_format']==[97,1]
                assert 'language' not in pack
                official={'de_de':2,'pl_pl':2,'zh_cn':2,'ru_ru':32}
                for code in ('de_de','pl_pl','zh_cn','ru_ru'):
                    values=json.loads(z.read(f'assets/catalogue/lang/{code}.json'))
                    assert not set(values)&protected(next(t for t in targets if t['mc']=='26.3' and t['loader']==r['loader']),code)
